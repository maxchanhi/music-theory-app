      ...
      parameters: {
        ...
        properties: {
          ...
          clef: {
            ...
            description: 'Optional: pick a specific clef (clef_minor only, overrides random)'
          },
          sharps: {
            type: 'integer',
            minimum: -6,
            maximum: 6,
            description: 'Optional: number of sharps (positive) or flats (negative) in key signature 0-6 (clef_minor only)'
          }
        },
        required: ['topic']
      }
...
${progressInfo}`;
  ...
  try {
    ...
    // If user requested a new topic but model refused to generate, do it ourselves
    let autoGeneratedQuestion = null;
    if (isNewRequest && !toolCallsThisTurn.includes('generate_question')) {
      // Detect the topic from the user message
      const topicNames = registry.listTopics().map(t => ({slug: t.topic, name: t.name.toLowerCase()}));
      const msg = message.toLowerCase();
      let matchedTopic = null;
      for (const t of topicNames) {
        if (msg.includes(t.slug) || msg.includes(t.name) || msg.includes(t.slug.replace(/_/g, ' '))) {
          matchedTopic = t.slug;
          break;
        }
      }
      if (matchedTopic) {
        try {
          const q = registry.generateQuestion(matchedTopic);
          if (q && q.displayData) {
            autoGeneratedQuestion = {
              topic: matchedTopic,
              displayData: q.displayData,
              correctAnswer: q.correctAnswer,
              questionText: q.questionText,
              choices: q.choices,
              answerFormat: q.answerFormat
            };
            // Add a fake tool call step so the tutor route picks up the displayData
            steps.push({
              step: -1,
              toolCalls: [{ name: 'generate_question', args: { topic: matchedTopic } }],
              results: [autoGeneratedQuestion]
            });
            toolCallsThisTurn.push('generate_question');
            // Override the reply with a question prompt
            const choiceText = q.choices ? `\n\nOptions: ${q.choices.join(', ')}` : '';
            finalReply = (finalReply || '') + '\n\n' + q.questionText + choiceText;
          }
        } catch (e) {
          console.warn('Auto-generate question failed for topic ' + matchedTopic + ': ' + e.message);
        }
      }
    }
